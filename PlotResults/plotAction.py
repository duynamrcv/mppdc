...
OBSTACLES = np.load("/home/duynam/MPC_formation_changing/Environment/environment_1.npy")
ROBOT_RADIUS = 0.3
SENSING_RADIUS = 3.0
...
path = "/home/duynam/MPC_formation_changing/Proposed/"
path0 = np.load(path+"path_0.npy")
path1 = np.load(path+"path_1.npy")
path2 = np.load(path+"path_2.npy")
...
ax = plt.axes()
num = 6
step = math.ceil(path0.shape[0]/num)
for j in [1,2,4,6]:
    i = j*step
    if i >= path0.shape[0]:
        i = -1
    ...
    plt.xlim([-9, 25])
    plt.tight_layout()
    fig.savefig("results/{}.pdf".format(j), format="pdf", bbox_inches="tight")
    plt.pause(1)
    ax.cla()
