...
plt.plot(path3[:,1], path3[:,2], label="Drone 3")
plt.plot(path4[:,1], path4[:,2], label="Drone 4")
plt.legend()
plt.xlabel("x (m)")
plt.ylabel("y (m)")
plt.grid(True)
...
distances = np.array([np.linalg.norm(path0[:,1:4]-path1[:,1:4], axis=1),
                      np.linalg.norm(path0[:,1:4]-path2[:,1:4], axis=1),
                      np.linalg.norm(path0[:,1:4]-path3[:,1:4], axis=1),
                      np.linalg.norm(path0[:,1:4]-path4[:,1:4], axis=1),
                      np.linalg.norm(path1[:,1:4]-path2[:,1:4], axis=1),
                      np.linalg.norm(path1[:,1:4]-path3[:,1:4], axis=1),
                      np.linalg.norm(path1[:,1:4]-path4[:,1:4], axis=1),
                      np.linalg.norm(path2[:,1:4]-path3[:,1:4], axis=1),
                      np.linalg.norm(path2[:,1:4]-path4[:,1:4], axis=1),
                      np.linalg.norm(path3[:,1:4]-path4[:,1:4], axis=1)]).T
plt.plot(path0[:,0], np.min(distances,axis=1), 'b-', label="")
...
num_state = path0[:,10] + path1[:,10] + path2[:,10] + path3[:,10] + path4[:,10]
...
if animation == True:
    ...
    for i in range(length):
        ...
        ax.plot(path0[:i,1], path0[:i,2], "-", label="Drone 0")
        ax.plot(path1[:i,1], path1[:i,2], "-", label="Drone 1")
        ax.plot(path2[:i,1], path2[:i,2], "-", label="Drone 2")
        ax.plot(path3[:i,1], path3[:i,2], "-", label="Drone 3")
        ax.plot(path4[:i,1], path4[:i,2], "-", label="Drone 4")

        # Plot current formation
        formation = np.array([[path0[i-1,1], path1[i-1,1], path2[i-1,1], path3[i-1,1], path4[i-1,1], path0[i-1,1]],
                              [path0[i-1,2], path1[i-1,2], path2[i-1,2], path3[i-1,2], path4[i-1,2], path0[i-1,2]]]).T
        ...
        ax.plot(a, b, '-k')
        a, b = getCircle(path3[i-1,1], path3[i-1,2], ROBOT_RADIUS)
        ax.plot(a, b, '-k')
        a, b = getCircle(path4[i-1,1], path4[i-1,2], ROBOT_RADIUS)
        ax.plot(a, b, '-k')
